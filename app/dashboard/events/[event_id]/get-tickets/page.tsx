...
export default async function Page(props: { params: { event_id: string } }) {
	...
	// if (discountedTicket) {
	// 	return <GetTicketPage event={event} ticket_info={ticket} isDiscounted={true} />;
	// } else {
	// 	return <GetTicketPage event={event} ticket_info={ticket} isDiscounted={false} />;
	// }
	...
	if (true) {
		return (
			<GetTicketPage
				event={event}
				ticket_info={mockDiscountedTicket}
				isDiscounted={true}
				user={user}
			/>
		);
